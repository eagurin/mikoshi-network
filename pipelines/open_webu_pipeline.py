...
class OpenAPI_R2R_Pipeline:
    ...
    async def inbound(self, body: dict) -> dict:
        ...
        try:
            ...
            openapi_result = await self.agent.arun(
                user_message
            ...
            r2r_context = await self.r2r_client.search_documents_async(
                query=user_message
            )  # Используем асинхронный метод
...
class Pipeline:
    ...
    async def inlet(self, body: dict, user: Optional[dict] = None) -> dict:
        ...
        response = {
            "role": "assistant",
            "content": f"Ваш запрос: {user_message}",
        }
...
async def pipeline_test():
    ...
    request_body = {
        ...
        "messages": [
            {"role": "user", "content": "Какая погода сегодня в Токио?"}
        ],
    }
